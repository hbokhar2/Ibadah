...
def get_paths() -> tuple[Path, Path, Path, Path]:
    ...
    ibadah_gd_path = ibadah_root_path / "gd" / "bin"
    ibadah_build_path = (ibadah_cxx_path.resolve().parents[0]) / "build"

    return ibadah_root_path, ibadah_cxx_path, ibadah_gd_path, ibadah_build_path
...
def cmake_build(ibadah_build_path: Path) -> bool:
    try:
        result = subprocess.run(
            ["cmake", "--build", str(ibadah_build_path)],
            check = True  
        )
...
if __name__ == "__main__":
    ...
    ibadah_root_path, ibadah_cxx_path, ibadah_gd_path, ibadah_build_path = get_paths()

    if not all(p.exists() for p in (ibadah_root_path, ibadah_cxx_path, ibadah_gd_path)):
    ...
    if str(command) == "all":
        if not cmake_clean(ibadah_build_path):
            sys.exit(1)
        if not cmake_generate_build_system(ibadah_cxx_path, ibadah_build_path):
            sys.exit(1)
        if not cmake_build(ibadah_build_path):
            sys.exit(1)
